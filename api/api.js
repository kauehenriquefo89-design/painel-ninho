...
async function fetchAll(path, token, extraParams = {}) {
  ...
  while (true) {
    ...
    const tamanho = extraParams.tamanho_pagina || 100;
    const extraSemTamanho = {...extraParams};
    delete extraSemTamanho.tamanho_pagina;
    const data = await caGet(path, token, { pagina, tamanho_pagina: tamanho, ...extraSemTamanho });
...
async function fetchFinancialData(access_token) {
  ...
  // Busca única com range amplo: captura itens por vencimento OU competência
  // O range de vencimento é amplo (2 anos) para não perder itens com competência no período
  // A separação DRE (competência) vs FC (vencimento) é feita no cliente pelos normalizadores
  // Busca por competência: vencimento com range de 3 anos para não perder nenhum item
  // O CA exige data_vencimento_de mas não nos interessa para filtrar a DRE
  ...
  // Busca por vencimento: para o Fluxo de Caixa
  const paramsFC = {
  ...
  const crDRE = await fetchAll("/v1/financeiro/eventos-financeiros/contas-a-receber/buscar", access_token, paramsDRE);
  await sleep(200);
  const cpDRE = await fetchAll("/v1/financeiro/eventos-financeiros/contas-a-pagar/buscar", access_token, paramsDRE);
  await sleep(200);
  const crFC  = await fetchAll("/v1/financeiro/eventos-financeiros/contas-a-receber/buscar", access_token, paramsFC);
  await sleep(200);
  const cpFC  = await fetchAll("/v1/financeiro/eventos-financeiros/contas-a-pagar/buscar", access_token, paramsFC);

  // UNION deduplicado por ID — DRE tem prioridade
  const mergeById = (a, b) => {
    const map = new Map();
  ...
  const cr = mergeById(crDRE, crFC);
  const cp = mergeById(cpDRE, cpFC);
  return { cr, cp };
}
