...
async function fetchFinancialData(access_token) {
  ...
  // Range amplo para capturar itens independente de qual data está no período
  // CA exige data_vencimento_de em todas as buscas
  const startAmplo = new Date(now.getFullYear()-1, 0, 1); // Jan do ano anterior
  const endAmplo   = new Date(now.getFullYear()+1, 11, 31); // Dez do próximo ano

  // Busca 1: por data_competencia (DRE) — vencimento com range amplo
  const paramsByComp = {
    data_competencia_de:  fmt(start),
    data_competencia_ate: fmt(now),
    data_vencimento_de:   fmt(startAmplo),
    data_vencimento_ate:  fmt(endAmplo),
  };
  // Busca 2: por data_vencimento (Fluxo de Caixa)
  const paramsByVenc = {
