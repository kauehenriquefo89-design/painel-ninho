...
async function fetchFinancialData(access_token) {
  ...
  // Vencimento com margem de 2 meses antes/depois do range de competência
  // Para capturar: competência Nov→ vencimento Jan (2 meses depois)
  const startVenc = new Date(start.getFullYear(), start.getMonth() - 2, 1);
  const endVenc   = new Date(now.getFullYear(),   now.getMonth()   + 2, 0);
  const params = {
    data_vencimento_de:   fmt(startVenc),
    data_vencimento_ate:  fmt(endVenc),
