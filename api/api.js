...
async function fetchAll(path, token, extraParams = {}) {
  ...
  while (true) {
    ...
    const data = await caGet(path, token, { pagina, tamanho_pagina: 500, ...extraParams });
...
async function fetchFinancialData(access_token) {
  const now   = new Date();
  // Usa o ÚLTIMO DIA do mês atual para capturar todos os lançamentos do mês
  // Ex: se hoje é 27/03, usa 31/03 para não perder lançamentos com competência 31/03
  ...
  // Busca única com range amplo de vencimento (3 anos) para garantir que todos
  // os itens com competência no período sejam retornados, independente do vencimento
  // A separação DRE (competência) vs FC (vencimento) é feita pelo normalizador no cliente
