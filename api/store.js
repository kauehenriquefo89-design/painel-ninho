// store.js — salva e serve dados do Excel via GitHub (arquivo data/excel.json no repositório)
...
const GITHUB_OFX_API = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/data/ofx.json`;

async function getOFXFromGitHub() {
  const token = process.env.GITHUB_TOKEN;
  if (!token) return null;
  try {
    const resp = await fetch(GITHUB_OFX_API, {
      headers: { "Authorization": `Bearer ${token}`, "Accept": "application/vnd.github+json" }
    });
    if (resp.status === 404) return null;
    if (!resp.ok) return null;
...
async function saveOFXToGitHub(payload, sha) {
  ...
  const content = Buffer.from(JSON.stringify(payload)).toString("base64");
  try {
    const body = { message: "Atualiza extrato OFX", content, ...(sha ? { sha } : {}) };
    const resp = await fetch(GITHUB_OFX_API, {
      method: "PUT",
...
module.exports = async (req, res) => {
  ...
  if (req.method === "POST" && action === "save") {
    ...
    // Busca SHA do arquivo existente (necessário para update)
